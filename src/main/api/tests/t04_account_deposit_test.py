...
class TestDepositAccount:
    def test_deposit_account_valid(
            ...
            user_with_account: tuple[CreateUserRequest, CreateAccountResponse]):
        ...
        amount = 5000
        api_manager.user_steps.deposit_account(user, account, amount)
        db_session.expire_all()

        account_from_db = Account.get_account_by_id(db_session, account.id)
        assert account_from_db is not None, 'Счёт пропал из БД после депозита'
        ...
        assert transaction_from_db.to_account_id == account.id, 'Депозит зачислен не на тот счёт'
    ...
    def test_deposit_account_invalid(
        ...
        amount = 999
        response = api_manager.user_steps.deposit_account_invalid(user, account, amount)
        ...
        transaction_from_db = Transaction.get_last_transaction_by_account_id(db_session, account.id)
